...
contract Comptroller is AccessControlEnumerableUpgradeable, ComptrollerStorage {
    ...
    function exitMarket(address cTokenAddress) external returns (uint256) {
        ...
        (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = ICToken(
            cToken
    ...
    function mintAllowed(
        ...
        require(
            maxSupply[cToken] == 0 ||
                (maxSupply[cToken] > 0 &&
                    ICToken(cToken).totalSupply().add_(mintAmount) <=
                    maxSupply[cToken]),
            "cToken > maxSupply"
        );
    ...
    function redeemAllowedInternal(
        ...
        (, , uint256 shortfall) = accountLiquidity
            .getHypotheticalAccountLiquidity(redeemer, cToken, redeemTokens, 0);
    ...
    function borrowAllowed(
        ...
        (, , uint256 shortfall) = accountLiquidity
            .getHypotheticalAccountLiquidity(borrower, cToken, 0, borrowAmount);
        require(shortfall <= 0, INSUFFICIENT_LIQUIDITY);
    ...
    function getAccountLiquidity(
        ...
        (uint256 err, uint256 liquidity, uint256 shortfall) = accountLiquidity
