...
contract AccountLiquidity is AccessControlEnumerableUpgradeable {
    ...
    function getHypotheticalAccountLiquidity(
        ...
        vars.equalAssetsGroupNum = IUnderwriterAdmin(underWriterAdmin)
            .getAssetGroupNum();
        AccountGroupLocalVars[] memory groupVars = new AccountGroupLocalVars[](
            vars.equalAssetsGroupNum
        );
        ...
        for (uint256 i = 0; i < assets.length; ++i) {
            ...
            vars.discountRate = Exp({
                mantissa: ICToken(asset).getDiscountRate()
            });
            ...
            vars.tokensToDenom = vars
                .tokensToDenom
                .mul_(vars.discountRate)
                .div_(1e18);
            ...
            vars.tokenDepositVal = vars.tokensToDenom.mul_ScalarTruncateAddUInt(
                vars.cTokenBalance,
                vars.tokenDepositVal
            );
            vars.tokenBorrowVal = vars.oraclePrice.mul_ScalarTruncateAddUInt(
                vars.borrowBalance,
                vars.tokenBorrowVal
            );
            if (asset == cTokenModify) {
                uint256 redeemVal = vars
                    .tokensToDenom
                    .mul_(redeemTokens)
                    .truncate();
            ...
            if (ICToken(asset).isCToken()) {
                groupVars[index].cTokenBalanceSum = vars.tokenDepositVal.add_(
                    groupVars[index].cTokenBalanceSum
                );
                groupVars[index].cTokenBorrowSum = vars.tokenBorrowVal.add_(
                    groupVars[index].cTokenBorrowSum
                );
            } else {
                groupVars[index].suTokenBalanceSum = vars.tokenDepositVal.add_(
                    groupVars[index].suTokenBalanceSum
                );
                groupVars[index].suTokenBorrowSum = vars.tokenBorrowVal.add_(
        ...
        for (uint8 i = 0; i < vars.equalAssetsGroupNum; ++i) {
            ...
            IUnderwriterAdmin.AssetGroup
                memory equalAssetsGroup = IUnderwriterAdmin(underWriterAdmin)
                    .getAssetGroup(groupVars[i].groupId);

            vars.intraCRate = Exp({
                mantissa: equalAssetsGroup.intraCRateMantissa
            });
            vars.intraMintRate = Exp({
                mantissa: equalAssetsGroup.intraMintRateMantissa
            });
            vars.intraSuRate = Exp({
                mantissa: equalAssetsGroup.intraSuRateMantissa
            });
            vars.interCRate = Exp({
                mantissa: equalAssetsGroup.interCRateMantissa
            });
            vars.interSuRate = Exp({
                mantissa: equalAssetsGroup.interSuRateMantissa
            });

            // absorb sutoken loan with ctoken collateral
            if (groupVars[i].suTokenBorrowSum > 0) {
                uint256 collateralizedLoan = vars
                    .intraMintRate
                    .mul_ScalarTruncate(groupVars[i].cTokenBalanceSum);
                if (groupVars[i].suTokenBorrowSum <= collateralizedLoan) {
                    // collateral could cover the loan
                    uint256 usedCollateral = groupVars[i].suTokenBorrowSum.div_(
                        vars.intraMintRate
                    );
                    groupVars[i].cTokenBalanceSum = groupVars[i]
                        .cTokenBalanceSum
                        .sub_(usedCollateral);
                    groupVars[i].suTokenBorrowSum = 0;
                } else {
                    // collateral could not cover the loan
                    groupVars[i].suTokenBorrowSum = groupVars[i]
                        .suTokenBorrowSum
                        .sub_(collateralizedLoan);
            ...
            if (groupVars[i].cTokenBorrowSum > 0) {
                uint256 collateralizedLoan = vars.intraCRate.mul_ScalarTruncate(
                    groupVars[i].cTokenBalanceSum
                );
                if (groupVars[i].cTokenBorrowSum <= collateralizedLoan) {
                    // collateral could cover the loan
                    uint256 usedCollateral = groupVars[i].cTokenBorrowSum.div_(
                        vars.intraCRate
                    );
                    groupVars[i].cTokenBalanceSum = groupVars[i]
                        .cTokenBalanceSum
                    ...
                    // collateral could not cover the loan
                    groupVars[i].cTokenBalanceSum = 0;
                    groupVars[i].cTokenBorrowSum = groupVars[i]
                        .cTokenBorrowSum
                        .sub_(collateralizedLoan);
                }
            }
            ...
            if (groupVars[i].suTokenBorrowSum > 0) {
                uint256 collateralizedLoan = vars
                    .intraSuRate
                    .mul_ScalarTruncate(groupVars[i].suTokenBalanceSum);
                if (groupVars[i].suTokenBorrowSum <= collateralizedLoan) {
                    // collateral could cover the loan
                    uint256 usedCollateral = groupVars[i].suTokenBorrowSum.div_(
                        vars.intraSuRate
                    );
                    groupVars[i].suTokenBalanceSum = groupVars[i]
                        .suTokenBalanceSum
                        .sub_(usedCollateral);
                    groupVars[i].suTokenBorrowSum = 0;
                } else {
        ...
        if (vars.sumBorrowPlusEffects > 0) {
            uint256 collateralizedLoan = targetVars
                .interCRate
                .mul_ScalarTruncate(targetGroup.cTokenBalanceSum);
            if (collateralizedLoan > vars.sumBorrowPlusEffects) {
                targetGroup.cTokenBalanceSum = targetGroup
                    .cTokenBalanceSum
                    .sub_(
                ...
                vars.sumBorrowPlusEffects = vars.sumBorrowPlusEffects.sub_(
                    collateralizedLoan
                );
        ...
        if (vars.sumBorrowPlusEffects > 0) {
            uint256 collateralizedLoan = targetVars
                .interSuRate
                .mul_ScalarTruncate(targetGroup.suTokenBalanceSum);
            if (collateralizedLoan > vars.sumBorrowPlusEffects) {
                targetGroup.suTokenBalanceSum = targetGroup
                    .suTokenBalanceSum
                    .sub_(
        ...
        if (vars.isSuToken) {
            ...
            vars.sumCollateral = vars.intraCRate.mul_ScalarTruncateAddUInt(
                targetGroup.cTokenBalanceSum,
                vars.sumCollateral
            );
        }
        vars.sumCollateral = vars.intraSuRate.mul_ScalarTruncateAddUInt(
