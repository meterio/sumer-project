...
contract CompLogic is AccessControlEnumerableUpgradeable {
    ...
    function _setCompSpeedInternal(
        address cToken,
        uint256 supplySpeed,
        uint256 borrowSpeed
    ) private {
    ...
    function _updateCompSupplyIndex(address cToken) private {
        ...
        uint256 deltaBlocks = uint256(blockNumber).sub_(
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            ...
            Double memory ratio = supplyTokens > 0
                ? _compAccrued.fraction(supplyTokens)
                : Double({mantissa: 0});
            supplyState.index = Double({mantissa: supplyState.index})
                .add_(ratio)
                .mantissa
                .safe224("new index exceeds 224 bits");
    ...
    function _updateCompBorrowIndex(
        address cToken,
        Exp memory marketBorrowIndex
    ) private {
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint256 borrowAmount = ICToken(cToken).totalBorrows().div_(
                marketBorrowIndex
            );
            uint256 _compAccrued = deltaBlocks.mul_(borrowSpeed);
            Double memory ratio = borrowAmount > 0
                ? _compAccrued.fraction(borrowAmount)
                : Double({mantissa: 0});
            borrowState.index = Double({mantissa: borrowState.index})
                .add_(ratio)
                .mantissa
                .safe224("new index exceeds 224 bits");
    ...
    function _distributeSupplierComp(address cToken, address supplier) private {
        ...
        Double memory deltaIndex = Double({
            mantissa: supplyIndex.sub_(supplierIndex)
        });
    ...
    function _distributeBorrowerComp(
        ...
        Double memory deltaIndex = Double({
            mantissa: borrowIndex.sub_(borrowerIndex)
        });

        uint256 borrowerAmount = ICToken(cToken)
            .borrowBalanceStored(borrower)
            .div_(marketBorrowIndex);
    ...
    function updateContributorRewards(address contributor) public {
        ...
        uint256 deltaBlocks = blockNumber.sub_(
            lastContributorBlock[contributor]
        );
        if (deltaBlocks > 0 && compSpeed > 0) {
            uint256 newAccrued = deltaBlocks.mul_(compSpeed);
            uint256 contributorAccrued = compAccrued[contributor].add_(
                newAccrued
            );
