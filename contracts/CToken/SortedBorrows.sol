...
contract SortedBorrows is AccessControlEnumerableUpgradeable, ISortedBorrows {
  ...
  function setComptroller(address _comptroller) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function _requireCallerIsComptroller() internal view {
    require(msg.sender == comptroller, 'SortedBorrows: Caller is not the Comptroller');
  }
}
