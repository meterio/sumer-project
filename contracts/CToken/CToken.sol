...
abstract contract CToken is CTokenStorage {
    ...
    function initialize(
        ...
        require(
            discountRateMantissa > 0 && discountRateMantissa <= 1e18,
            "rate must in [0,100]"
        );
    ...
    function transferTokens(
        ...
        uint256 allowed = IComptroller(comptroller).transferAllowed(
            address(this),
            src,
            dst,
            tokens
        ...
        if (allowed != 0) {
            Error.COMPTROLLER_REJECTION.failOpaque(
    ...
    function balanceOfUnderlying(
        ...
        (MathError mErr, uint256 balance) = exchangeRate.mulScalarTruncate(
            accountTokens[owner]
        );
    ...
    function borrowBalanceStoredInternal(
         ...
         */
        (mathErr, principalTimesIndex) = borrowSnapshot.principal.mulUInt(
            borrowIndex
        );
        if (mathErr != MathError.NO_ERROR) {
            return (mathErr, 0);
        ...
        (mathErr, result) = principalTimesIndex.divUInt(
        ...
        returns (MathError, uint256)
    {
        ...
        if (_totalSupply == 0) {
            ...
            (mathErr, cashPlusBorrowsMinusReserves) = totalCash.addThenSubUInt(
                totalBorrows,
                totalReserves
            );
            if (mathErr != MathError.NO_ERROR) {
            ...
            (mathErr, exchangeRate) = cashPlusBorrowsMinusReserves.getExp(
                _totalSupply
            );
            if (mathErr != MathError.NO_ERROR) {
                return (mathErr, 0);
    ...
    function accrueInterest() public virtual override returns (uint256) {
        ...
        (MathError mathErr, uint256 blockDelta) = currentBlockNumber.subUInt(
        ...
        (mathErr, simpleInterestFactor) = Exp({mantissa: borrowRateMantissa})
            .mulScalar(blockDelta);
        if (mathErr != MathError.NO_ERROR) {
            return
        ...
        (mathErr, interestAccumulated) = simpleInterestFactor.mulScalarTruncate(
            borrowsPrior
        );
        if (mathErr != MathError.NO_ERROR) {
            return
        ...
        (mathErr, totalReservesNew) = Exp({mantissa: reserveFactorMantissa})
            .mulScalarTruncateAddUInt(interestAccumulated, reservesPrior);
        if (mathErr != MathError.NO_ERROR) {
            return
                Error.MATH_ERROR.failOpaque(
                    FailureInfo
        ...
        (mathErr, borrowIndexNew) = simpleInterestFactor
            .mulScalarTruncateAddUInt(borrowIndexPrior, borrowIndexPrior);
        if (mathErr != MathError.NO_ERROR) {
            return
                Error.MATH_ERROR.failOpaque(
                    FailureInfo
    ...
    function mintFresh(
        ...
        uint256 allowed = IComptroller(comptroller).mintAllowed(
            address(this),
            minter,
            mintAmount
        );
        ...
        (vars.mathErr, vars.mintTokens) = vars
            .actualMintAmount
            .divScalarByExpTruncate(Exp({mantissa: vars.exchangeRateMantissa}));
        require(
         ...
         */
        (vars.mathErr, vars.totalSupplyNew) = totalSupply.addUInt(
            vars.mintTokens
        );
        require(
            vars.mathErr == MathError.NO_ERROR,
        ...
        (vars.mathErr, vars.accountTokensNew) = accountTokens[minter].addUInt(
            vars.mintTokens
        );
        require(
            vars.mathErr == MathError.NO_ERROR,
    ...
    function redeemFresh(
        ...
        if (redeemTokensIn > 0) {
            ...
            (vars.mathErr, vars.redeemAmount) = Exp({
                mantissa: vars.exchangeRateMantissa
            }).mulScalarTruncate(redeemTokensIn);
            if (vars.mathErr != MathError.NO_ERROR) {
                Error.MATH_ERROR.failOpaque(
            ...
            (vars.mathErr, vars.redeemTokens) = redeemAmountIn
        ...
        uint256 allowed = IComptroller(comptroller).redeemAllowed(
            address(this),
            redeemer,
            vars.redeemTokens
        );
         ...
         */
        (vars.mathErr, vars.totalSupplyNew) = totalSupply.subUInt(
            vars.redeemTokens
        );
        if (vars.mathErr != MathError.NO_ERROR) {
            Error.MATH_ERROR.failOpaque(
        ...
        (vars.mathErr, vars.accountTokensNew) = accountTokens[redeemer].subUInt(
            vars.redeemTokens
        );
        if (vars.mathErr != MathError.NO_ERROR) {
            Error.MATH_ERROR.failOpaque(
        ...
        if (isCToken && (getCashPrior() < vars.redeemAmount)) {
            Error.TOKEN_INSUFFICIENT_CASH.fail(
        ...
        /* We call the defense hook */
        IComptroller(comptroller).redeemVerify(
            address(this),
            redeemer,
            vars.redeemAmount,
            vars.redeemTokens
    ...
    function borrowFresh(
        ...
        uint256 allowed = IComptroller(comptroller).borrowAllowed(
            address(this),
            borrower,
            borrowAmount
        );
        ...
        (vars.mathErr, vars.accountBorrowsNew) = vars.accountBorrows.addUInt(
            borrowAmount
        );
        if (vars.mathErr != MathError.NO_ERROR) {
            return
        ...
        (vars.mathErr, vars.totalBorrowsNew) = totalBorrows.addUInt(
            borrowAmount
        );
        if (vars.mathErr != MathError.NO_ERROR) {
            return
    ...
    function repayBorrowFresh(
        ...
        uint256 allowed = IComptroller(comptroller).repayBorrowAllowed(
            address(this),
            payer,
            borrower,
            repayAmount
         ...
         */
        (vars.mathErr, vars.accountBorrowsNew) = vars.accountBorrows.subUInt(
        ...
        (vars.mathErr, vars.totalBorrowsNew) = totalBorrows.subUInt(
            vars.actualRepayAmount
        );
        require(
            vars.mathErr == MathError.NO_ERROR,
    ...
    function liquidateBorrowFresh(
        ...
        uint256 allowed = liquidateBorrowAllowed(
            address(cTokenCollateral),
            liquidator,
            borrower,
            repayAmount
        ...
        if (
            ICToken(cTokenCollateral).accrualBlockNumber() != getBlockNumber()
        ) {
            Error.MARKET_NOT_FRESH.fail(
        ...
        if (borrower == liquidator) {
            Error.INVALID_ACCOUNT_PAIR.fail(
        ...
        if (repayAmount == 0) {
            Error.INVALID_CLOSE_AMOUNT_REQUESTED.fail(
        ...
        (
            ...
            uint256 seizeTokens
        ) = liquidateCalculateSeizeTokens(cTokenCollateral, actualRepayAmount);
        require(
        ...
        if (cTokenCollateral == address(this)) {
            ...
            seizeError = ICToken(cTokenCollateral).seize(
                liquidator,
                borrower,
                seizeTokens
            );
    ...
    function seizeInternal(
         ...
         */
        (vars.mathErr, vars.borrowerTokensNew) = accountTokens[borrower]
            .subUInt(seizeTokens);
        if (vars.mathErr != MathError.NO_ERROR) {
            Error.MATH_ERROR.failOpaque(
        ...
        vars.protocolSeizeTokens = seizeTokens.mul_(
        ...
        vars.protocolSeizeAmount = Exp({mantissa: vars.exchangeRateMantissa})
            .mul_ScalarTruncate(vars.protocolSeizeTokens);
        ...
        (vars.mathErr, vars.liquidatorTokensNew) = accountTokens[liquidator]
            .addUInt(vars.liquidatorSeizeTokens);
        if (vars.mathErr != MathError.NO_ERROR) {
            Error.MATH_ERROR.failOpaque(
    ...
    function _setComptroller(
        ...
        require(
            IComptroller(newComptroller).isComptroller(),
    ...
    function _setReserveFactorFresh(
        uint256 newReserveFactorMantissa
    ) internal onlyAdmin returns (uint256) {
        // Verify market's block number equals current block number
        if (accrualBlockNumber != getBlockNumber()) {
            Error.MARKET_NOT_FRESH.fail(
    ...
    function _setInterestRateModelFresh(
        ...
        require(
            IInterestRateModel(interestRateModel).isInterestRateModel(),
            "marker method returned false"
        );
    ...
    function isDeprecated() public view returns (bool) {
        return
            ...
            IUnderwriterAdmin(IComptroller(comptroller).underWriterAdmin())
                ._getBorrowPaused(address(this)) &&
    ...
    function liquidateBorrowAllowed(
        ...
        require(
            IComptroller(comptroller).isListed(address(this)) &&
                IComptroller(comptroller).isListed(cTokenCollateral),
            "MARKET_NOT_LISTED"
        );
        ...
        if (isDeprecated()) {
            ...
            (, , uint256 shortfall) = IComptroller(comptroller)
                .getHypotheticalAccountLiquidity(borrower, address(this), 0, 0);

            require(shortfall > 0, "insufficient shortfall");

            /* The liquidator may not repay more than what is allowed by the closeFactor */
            uint256 maxClose = Exp({
                mantissa: IComptroller(comptroller).closeFactorMantissa()
    ...
    function liquidateCalculateSeizeTokens(
        ...
        uint256 priceBorrowedMantissa = IPriceOracle(oracle).getUnderlyingPrice(
            address(address(this))
        );
        uint256 priceCollateralMantissa = IPriceOracle(oracle)
            .getUnderlyingPrice(address(cTokenCollateral));
        require(
        ...
        numerator = Exp({
            mantissa: IComptroller(comptroller).liquidationIncentiveMantissa()
        }).mul_(Exp({mantissa: priceBorrowedMantissa}));
    ...
    function getAccountBorrows(
        address account
    ) public view returns (uint256 principal, uint256 interestIndex) {
