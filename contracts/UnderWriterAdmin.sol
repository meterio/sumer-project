...
contract UnderwriterAdmin is UnderwriterStorage, ComptrollerErrorReporter {
    ...
    function setEqAssetGroup(
        ...
        if (msg.sender != admin) {
            return
                fail(
                    Error.UNAUTHORIZED,
                    FailureInfo.SET_EQUAL_ASSET_GROUP_OWNER_CHECK
                );
        }
    ...
    function getEqAssetGroup(uint8 groupId)
        public
        view
        returns (EqualAssets memory)
    {
    ...
    function _getMarketBorrowCap(CToken cToken)
        external
        view
        returns (uint256)
    {
