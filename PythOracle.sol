...
// File contracts/Oracle/PythOracle.sol
...
contract PythOracle {
  ...
  event SetFeed(address indexed cToken_, bytes32 feedId, address addr, string name);
  ...
  function setFeedId(address cToken_, bytes32 feedId, address addr, string memory name) public onlyOwner {
    _setFeed(cToken_, feedId, addr, name);
  }

  function _setFeed(address cToken_, bytes32 feedId, address addr, string memory name) private {
    require(addr != address(0), 'Address is Zero!');
    require(feedId != bytes32(0), 'feedId is Zero!');

    FeedData memory feedData = FeedData({feedId: feedId, addr: addr, name: name});
  ...
  function getUnderlyingPrice(address cToken_) public view returns (uint256) {
    ...
    require(feed.feedId != bytes32(0), 'feedId not exist!');

    PythStructs.Price memory price = IPyth(feed.addr).getPriceUnsafe(feed.feedId);

    uint256 decimals = uint256(DECIMALS - uint32(price.expo) - 18);
