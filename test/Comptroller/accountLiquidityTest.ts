...
import { makeComptroller, makePriceOracle, makeCToken, makeToken, error, failureInfo, enterMarkets } from '../Utils/Compound';
...
describe('Comptroller', () => {
  ...
  describe('liquidity', () => {
    ...
    it("allows entering 3 markets, supplying to 2 and borrowing up to collateralFactor in the 3rd", async () => {
      ...
      const comptroller = await ethers.getContractAt("Comptroller",await cToken1.comptroller()) as Comptroller;
      const cToken2 = await makeCToken(root,'erc20','supportMarket', comptroller.address, up2);
      const cToken3 = await makeCToken(root,'erc20','supportMarket', comptroller.address,  up3);
  ...
  describe("getHypotheticalAccountLiquidity", () => {
    ...
    it("returns collateral factor times dollar amount of tokens minted in a single market", async () => {
      ...
      const cToken = await makeCToken(root,'erc20','supportMarket',  exchangeRate, underlyingPrice);
