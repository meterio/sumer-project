...
task('jv2', 'deploy InterestRateModel contracts')
    ...
    .setAction(async ({ blocks, base, mul, jump, kink, rpc, pk, gasprice }, { ethers, run, network }) => {
        ...
        const interestRateModel = await run("d", {
            ...
            args: [
                blocks,
                ethers.utils.parseUnits(base, MANTISSA_DECIMALS),
                ethers.utils.parseUnits(mul, MANTISSA_DECIMALS),
                ethers.utils.parseUnits(jump, MANTISSA_DECIMALS),
                ethers.utils.parseUnits(kink, MANTISSA_DECIMALS),
                wallet.address
            ],
            rpc: rpc,
            pk: pk,
