...
task('hal', 'get Hypothetical Account Liquidity')
    ...
    .setAction(async ({ account, json, ctoken, redeem, borrow, rpc, pk, gasprice }, { ethers, run, network }) => {
        ...
        const accountLiquidity = await comptroller.getHypotheticalAccountLiquidity(account, ctoken, redeem, borrow, override);
        ...
        const ua = await ethers.getContractAt("UnderwriterAdmin", config.underwriterAdmin.address, wallet) as UnderwriterAdmin;
        ...
        for (let i = 0; i < assets.length; i++) {
            ...
            let [err, cTokenBalance, borrowBalance, exchangeRateMantissa] = await assetToken.getAccountSnapshot(account, override);
        ...
        for (let i = 0; i < vars.equalAssetsGroupNum; i++) {
            ...
            if (groupVars[i].groupId == await comptroller.marketGroupId(ctoken)) {
                ...
                vars.sumCollateral = mul_ScalarTruncateAddUInt(vars.interCRate,
                ...
                vars.sumCollateral = mul_ScalarTruncateAddUInt(vars.interSuRate,
